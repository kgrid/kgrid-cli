...
describe('download', () => {
  ...
  td.when(del(td.matchers.anything())).thenResolve(() => {
  });
  ...
  describe('with files', () => {
    ...
    test
      .command([`download`, `-d`, `${destination}`,`-f`, `${remoteFilePath}`])
      .it('should delete the temporary directory', (context) => {
        td.verify(del([path.join(destination, 'tmp')]))
      });
  });
  describe('with manifest', () => {
    ...
    test
      .command([`download`, `-d`, `${destination}`,`-m`, `${remoteManifestPath}`])
      .it('should delete the temporary directory', (context) => {
        td.verify(del([path.join(destination, 'tmp')]))
      });
  });
